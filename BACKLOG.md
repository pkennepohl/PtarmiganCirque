...
| ⏳ | 🟡 | **Per-row `[~]` baseline-curve toggle column alignment across all node types (USER-FLAGGED)** | USER-FLAGGED at end of Phase 4t. CS-36 (Phase 4r) renders the per-row `[~]/[–]` BASELINE toggle ONLY on BASELINE rows (`if node.type == NodeType.BASELINE` in `_populate_node_row`). The result: rows of different node types have different optional-cell counts in the always-visible cluster, so the right-sidebar columns don't align across rows. The user has flagged that the toggle cell should be present for every node type listed in the table, but greyed out (`state="disabled"`) for nodes where there's no baseline curve to show (UVVIS / NORMALISED / SMOOTHED / SECOND_DERIVATIVE / PEAK_LIST). Concretely: drop the `if node.type == NodeType.BASELINE` gate; always pack the button; for non-BASELINE rows configure `state="disabled"` + `text="–"` (or a different glyph that signals "not applicable" rather than "off") + skip the click binding. Likely also wants a tooltip on the disabled state explaining "No baseline curve for this node type" (reuses the CS-42 Tooltip pattern + the empty-string-sentinel CS-43 rotation). Touches `scan_tree_widget._populate_node_row` only. Test coverage: `TestPerNodeBaselineCurveToggle.test_uvvis_row_does_not_have_toggle_button` flips its assertion (the button DOES appear, but state="disabled"); the existing positive BASELINE-row test stays. Cross-refs CS-43's empty-string-sentinel Tooltip pattern — the same shape works here for the disabled row tooltip |
...
| ⏳ | 🟡 | **Dynamic sidebar auto-width for long node labels (USER-FLAGGED)** | USER-FLAGGED at end of Phase 4u (step 5 elicitation). Today the right-sidebar panel width is set once at tab build via `tk.PanedWindow` sash position; long node labels (truncated to 32 chars per CS-33, with hover tooltip showing the full text) still create natural-width pressure that overflows the visible cell area, requiring the user to manually drag the sash wider. The user has asked whether the sidebar should auto-widen to accommodate longer labels. **Architecture proposal:** at end of `_rebuild` in ScanTreeWidget (or via a callback wired into the host's PanedWindow), measure the maximum label-cell natural width across all visible rows and bump the sash position to fit when the current width is smaller — with an upper cap (e.g. 35% of the host pane) so a single pathological label doesn't take over. Cross-refs CS-33 (label truncation) — once truncation kicks in, the natural width is bounded, but typical labels under the cap still vary by ~10x in pixel width. Cross-refs CS-30 (canvas-width responsive helper) — that fix made resize behaviour correct *given* a fixed sash; this entry is the orthogonal "set the sash dynamically" follow-up. Touches `scan_tree_widget` (new measurement helper), the host integration (PanedWindow sash setter), and likely a new test class `TestScanTreeWidgetDynamicSashWidth`. **Lock decision needed in the implementing session:** auto-widen on every rebuild vs only when label-cell natural width has grown vs only on first display; user-overridable via manual drag (probably yes — manual drag should pin the sash until next session) |
...
| ⏳ | 🔴 | **Adjustable sidebar still not working — fix next, revisit every cell + minimum width (USER-FLAGGED bumped from 🟡 to 🔴 in Phase 4v)** | USER-FLAGGED at end of Phase 4u as 🟡 (Phase 4u friction #5; "Dynamic sidebar auto-width for long node labels"); user re-flagged at end of Phase 4v as still-broken-and-blocking-work. Bumped to 🔴. Expanded scope per Phase 4v step 5: the right sidebar is to be revisited cell-by-cell at the same time as the auto-width fix, with consideration for a per-cell **minimum width** so the always-visible cluster never overflows below readability. The original Phase 4u carry-forward framing remains accurate (auto-bump PanedWindow sash to fit widest label-cell natural width) but the implementing session should also: walk every cell defined in CS-26's responsive helper, set per-cell pixel-width minimums, document the sidebar's full cell vocabulary, and pin a "minimum sidebar width" constant. Cross-refs CS-30 (canvas-width responsive helper — Phase 4p), CS-33 (label truncation — Phase 4q), and the future label-truncation-cap-from-canvas-width entry. **Affected:** `scan_tree_widget` (every cell helper), the host's `tk.PanedWindow` sash setter, possibly `uvvis_tab.py`'s pane initialisation. New tests in `TestScanTreeWidgetSidebarMinimumWidth` + the existing dynamic-sash class once it lands |
...
1. 🟡 **Per-row `[~]` toggle column alignment across all node
   ...
   every row but greyed out when not applicable. **Cross-ref:**
   see the new register entry "Per-row `[~]` baseline-curve
   toggle column alignment across all node types
   (USER-FLAGGED)" above for the architecture (drop the type
   gate in `_populate_node_row`; configure `state="disabled"`
   + tooltip explaining "No baseline curve for this node
   type" via the CS-42 Tooltip pattern). Touches
   `scan_tree_widget._populate_node_row` only. Test impact:
   `TestPerNodeBaselineCurveToggle.test_uvvis_row_does_not_have_toggle_button`
   flips its assertion (button DOES appear, but state="disabled").
...
5. 🔴 **Dynamic sidebar auto-width for long labels — STILL OPEN
   ...
   natural width. User re-flagged at end of Phase 4v as still-
   broken-and-blocking-work. Scope expanded: revisit every cell
   in the sidebar at the same time, with a per-cell minimum
   width so the always-visible cluster never overflows below
   readability. **Cross-ref:** see the bumped register entry
   above ("Adjustable sidebar still not working — fix next");
   cross-refs CS-30 + CS-33.
...
2. 🔴 **Adjustable sidebar still not working — bumped to 🔴
   (USER-FLAGGED).** See Phase 4u friction #5 above (canonical
   entry — re-flagged at end of Phase 4v, scope expanded:
   revisit every cell + minimum widths, fix next). **Cross-
   ref:** see the bumped register entry above.
