...
4. 🟡 ~~**USER-FLAGGED Apply-to-all icon on per-axis tabs.**~~
   ✅ Resolved Phase 4at (CS-73). Per-row ∀ buttons on every
   per-axis Settings widget except `axis_label_override`.
   See canonical register entry above.
...
6. 🟡 ~~**USER-FLAGGED Apply-to-all icon on per-axis tabs.**~~
   ✅ Resolved Phase 4at (CS-73). Cross-ref Phase 4ak
   friction #4 (canonical resolution record).
...
1. 🟢 ~~**`NODE_GROUP_MEMBERS_CHANGED` is missing from
   ...
   Phase 4as artifact).**~~ ✅ Resolved Phase 4at (item α) —
   commit `2c5a255` adds `GraphEventType.NODE_GROUP_MEMBERS_CHANGED`
   to the `_redraw`-trigger tuple at
   `uvvis_tab.py:_on_graph_event`, and commit `0498ebe` pins
   three regression sentinels in
   `TestUVVisTabNodeGroupMembersRedrawPhase4at` (redraw fires;
   CS-72 wiring still intact; other redraw triggers still
   fire). Grouping or ungrouping nodes now repaints the plot
   in lockstep with the dialog's listbox refresh.
...
### Friction points carried forward from Phase 4at

These are concrete obstacles the next Phase 4 session will hit.
Phase 4at bundled **three landings** in one session: (i) CS-73
— per-row ∀ Apply-to-all buttons on every per-axis Settings
widget except `axis_label_override` (USER-FLAGGED from Phase 4ak,
resolved with the canonical record updated above); (ii) item α
— `NODE_GROUP_MEMBERS_CHANGED` added to `_on_graph_event`'s
`_redraw` trigger tuple (Phase 4as carry-forward); (iii) item
β — collapse the inline provenance pane's "two lines per step"
into a single `↳ op [engine ver] → output_label` line (USER-
surfaced at step 5 — *"every step actually takes two lines to
show ... I want to collapse that so each step is only a single
line"*). 32 net new tests pinned (20 unit + 6 integration on
the dialog/host axis-broadcast surface; 6 unit on the history
collapse). 1473 tests, all green (1441 baseline + 32 new). CS-73
adopts the CS-05 `style_dialog` ∀-glyph convention with a fresh
greying composition (D8): CS-71's `_apply_axis_autoscale_greying`
and CS-70's `_apply_secondary_x_link_greying` now both walk the
new `_axis_apply_to_all_buttons[(role, key)]` registry so a ∀
next to an inert widget is itself inert. The `_suspend_writes`
guard + manual `_working` write + single `_apply_changes_live`
fire (D4) is the load-bearing efficiency tweak — without it
defer-commit Entries would require per-target focus events and
non-deferred widgets would fire on_apply once per target. PTMG
schema unchanged — no `PTMG_FORMAT_VERSION` bump. **No new
carry-forward items.** All three landings closed cleanly; the
implementation passes surfaced no new friction.

1. ✅ **No new carry-forward.** Phase 4at touched three
   surfaces (plot_settings_dialog, uvvis_tab, scan_tree_widget)
   plus their test modules; each landing was scoped tight and
   nothing surfaced during implementation that needs deferral.
...
3. 🟡 ~~**USER-FLAGGED Accessibility sub-axis C — Keyboard
   ...
   ✅ **Resolved Phase 4az (CS-78).** Cross-ref canonical
...
1. 🟡 ~~**USER-FLAGGED Accessibility sub-axis B — Colour-blind
   ...
   **medium**.~~ ✅ Resolved in Phase 4ay (CS-77). Two phase
   commits — `0f29320` (node_styles palette opt-in shell) and
   `7bd2b2c` (Accessibility tab + persistence integration).
   See sub-axis B canonical register row above for the full
   deliverable summary.
...
2. 🟡 ~~**USER-FLAGGED Accessibility sub-axis C — Keyboard
   ...
   **medium**.~~ ✅ **Resolved Phase 4az (CS-78).** New
   `accessibility.bind_shortcut` is the recipe-canonical entry
   point (`widget.bind` + `register_shortcut` in one call);
   CS-75 D6 additive sub-clause preserves Tooltip-only
   discoverability for hover-capable widgets while the
   Accessibility-tab table covers container widgets without a
   natural hover surface. Cross-ref canonical sub-axis C row
...
11. 🟢 ~~**CS-75 D3 lock relaxation precedent set
    ...
    Reinforced in Phase 4ay — CS-21 D3 relaxed additively
    (new `SPECTRUM_PALETTE_NAME` + `active_palette()` joined
    `node_styles` without removing `SPECTRUM_PALETTE`) AND
    CS-75 D1 introduced its first surface (the Accessibility
    tab in `PlotConfigDialog`). The "locks enumerate
    Phase-4aw-known surfaces; step-5 elicitation is the
    canonical relaxation gate" pattern now has two consecutive
    confirming examples (4ax binah expansion + 4ay CS-21 D3
    additive). No further bookkeeping needed.

### Friction points carried forward from Phase 4ay

These are concrete obstacles the next Phase 4 session will hit.
Phase 4ay landed the second sub-batch of the CS-75 Accessibility
umbrella — sub-axis B (Colour-blind palette opt-in + new
"Accessibility" tab in `PlotConfigDialog` introducing the CS-75 D1
first surface). Two phase commits + this bookkeeping commit. 1620
tests, all green (1584 baseline + 36 net new). PTMG_FORMAT_VERSION
unchanged — schema-additive via the new
`plot_defaults["accessibility"]["palette"]` slot. **Do not fix
until the relevant subsequent Phase 4 session.**

1. 🟡 ~~**USER-FLAGGED Accessibility sub-axis C — Keyboard
   shortcuts first batch + KEYBINDINGS.md (Phase 4az candidate,
   NEXT-UP).** Scope locked Phase 4aw (CS-75 D3 / D6); see
   canonical sub-axis C row in the Phase 4q register table.
   First batch candidates: Ctrl+G group, Ctrl+Shift+G ungroup,
   Delete discard, F2 rename. Per-component `bind` pattern
   (CS-74 / CS-76 precedent — `bind_escape_to_close` is the
   canonical adapter shape). Phase 4ax's
   `attach_shortcut_tooltip` is the canonical Tooltip entry
   point for shortcut discoverability — every Phase 4az
   shortcut-bearing widget routes through it. New
   `KEYBINDINGS.md` sister doc carries the registered shortcut
   table — every sub-batch updates it in the bookkeeping
   commit. Reasoning level: **medium**.~~ ✅ **Resolved Phase
   4az (CS-78).** All four candidate gestures (F2 / Delete /
   Ctrl+G / Ctrl+Shift+G) landed on the scan_tree as the
   canonical first batch; new KEYBINDINGS.md sister doc seeded
   with the Scan tree section; new `bind_shortcut` recipe
   captures `widget.bind` + `register_shortcut` in one call so
   the binding and the discoverability table cannot drift.
   CS-75 D6 preserved as an additive sub-clause for hover-less
   container widgets. Cross-ref canonical sub-axis C row in
...
2. 🟡 ~~**USER-FLAGGED Accessibility sub-axis D — Font-scale
   ...
   helper + `accessibility.font_scale: float` key in
   `_USER_DEFAULTS["accessibility"]` (Phase 4ay introduced
   the sub-dict shell; this sub-batch fills the second key).
   Persists via the CS-46 schema-additive slot already in
...
3. 🟡 **USER-FLAGGED — `[engine engine_version]` provenance
   ...
   step-5 elicitation).** User: "what does `[internal 0.1.0]`
   mean? It is listed for all operations on a datastream and
   it seems superfluous if ALL operations are all listed in
   this way." Rendered at `scan_tree_widget.py:2081` (history
   line) + `:2099` (current-op line) from
   `f"[{op.engine} {op.engine_version}]"`. `engine` defaults
   to `"internal"` for Ptarmigan-produced ops (future-proofed
   for external engines like Athena / BL processing software);
   `engine_version` is sourced from `version.__version__`
   (currently frozen at `"0.1.0"`). The bracket IS provenance
   — but with one engine + one version in the world, it's
   100% redundant on every row today. **Two improvements:**
   (a) suppress the bracket when uniform across visible rows
   (collapse to a single header line "engine: internal 0.1.0
   (all)" or move to a hover tooltip / Details pane); (b)
   adopt a meaningful version-bump policy so 0.1.0 ≠ all
   builds. Cross-refs CS-42 (Tooltip is the natural home if
   we choose the hover-only route), the still-open external
   plot style presets row (which references external Jupyter
   notebook code — first credible non-internal engine candidate
   in the wild). Reasoning level: **low-medium** for the
   display suppression; **medium** for a version-bump policy
...
4. 🟢 **USER-FLAGGED — Per-axis font customization with
   ...
   other font style information) should by default be the
   same as the other axis." Today every dialog font is a
   literal `font=("", N, ...)`; CS-75 D4 (sub-axis D font
   scale, Phase 4ba candidate) introduces a single global
   multiplier, NOT per-axis customization. This request is
   broader: each axis tab gains per-axis font-family /
   font-size / font-style controls, and twin-axis pairings
   (wavelength↔energy via CS-44's `_resolve_non_primary_y_label`
   + secondary-x twin pairing in the renderer) default
   to inherit the primary axis's font settings with an
   explicit "override" toggle. **Architecture proposal
   (lock pending):** new `font_family` / `font_size` /
   `font_weight` / `font_style` keys per axis under
   `_FACTORY_DEFAULTS["axes"][role]`; per-axis tab grows a
   "Font" LabelFrame; twin-axis defaulting via a "Inherit
   from {primary_role}" checkbox that, when checked, hides
   the per-axis font widgets and reads through to the
   inherited axis's values; renderer reads the resolved
   value at `set_xlabel` / `set_ylabel` time. **Lock
   decisions for the implementing session:** (i) does this
   replace Phase 4ba's global font-scale (sub-axis D), or
   compose with it (per-axis size in points × global scale
   multiplier)? (ii) what about title + tick labels — do
   they get per-axis Font controls too, or stay global? (iii)
   how does the override toggle interact with Reset/Factory
   Reset? **Affected:** `_FACTORY_DEFAULTS["axes"]` schema
   (CS-46 schema-additive; PTMG_FORMAT_VERSION unchanged if
   nested under the existing `axes` sub-dict),
   `plot_settings_dialog.py` per-axis tabs (new Font
   LabelFrame + inherit toggle), the renderer at every
   `set_xlabel` / `set_ylabel` / `tick_params` site. Cross-refs
   the existing Phase 4ay carry-forward sub-axis D row above
   (likely compose — sub-axis D ships first as the global
   scale, this request lands later as the per-axis override).
   Reasoning level: **high** (cross-cutting schema +
   dialog + renderer).

5. 🟢 **`ttk.Combobox` readonly initial-value clear under
   heavy Tk state (Claude-surfaced, Phase 4ay artifact).**
   The Accessibility-tab palette Combobox's textvariable-supplied
   initial value occasionally cleared to `""` under the full
   `run_tests.py` run (`test_palette_combobox_initial_value_is_default_label`
   fails) but ALWAYS passed when `test_plot_settings_dialog`
   ran in isolation. Defensive `cb.set(current_label)`
   immediately after `ttk.Combobox(textvariable=var, …)`
   construction (and before `trace_add` registers the writer)
   fixed it without changing the public widget shape. **Pattern
   to adopt going forward:** any new readonly ttk.Combobox
   with a textvariable-supplied initial value should explicitly
   `cb.set(initial_value)` after construction; the textvariable
   alone is not load-bearing under cross-module Tk state. No
   register entry — captured in CS-77's "Defensive Combobox
   re-seed" subsection.

6. 🟢 **Out-of-scope Toplevel inventory carry-forward
   (continued from Phase 4ax friction #4).** Phase 4ax wired
   ten of ~30 `tk.Toplevel(` sites repo-wide; Phase 4ay
   added no new wirings (focused on palette only). Remaining
   sites for future Escape-recipe expansion:
   * `plot_widget.py` — 7 user-facing sites (lines 1826,
     1948, 2169, 2304, 2457, 3355, 3996).
   * `xas_analysis_tab.py` — 2 sites (lines 684, 1300).
   * `ledge_normalizer.py` — 1 site (line 1218).
   * `nbo_viewer_app.py` — 13 sites (sibling-app; out of
     scope until/unless that app integrates further).
   * `tooltip.py:78` + `plot_widget.py:123` — Tooltip
     internal infra (CS-42; not user-facing).
   No register entry — captured in CS-76's out-of-scope
   inventory subsection. Reasoning level: **low** if pursued
   standalone, **medium** if rolled into Phase 4az / 4ba as
   opportunistic cross-cut wiring.

7. 🟢 **Hard-coded Tooltip text on the NodeStylesDialog
   Combobox (continued from Phase 4ax friction #6).**
   `attach_shortcut_tooltip` text is `"Ctrl+↑/↓ to step
   through nodes"`. If Phase 4az adds Ctrl+Home / Ctrl+End,
   rotate via `self._combobox_tooltip.update_text(...)`.
   The handle is stored on the dialog instance for exactly
   this rotation. No register entry.

8. 🟢 **Phase 4av carry-forward items #2 / #3 / #4 still
   open (continued from Phase 4ax friction #7 / Phase 4aw
   friction #5).** (#2) "Reset" button label semantics
   shift; (#3) `_set_universal_disabled` root readonly-Combobox
   latent bug; (#4) DISCARDED glyph distinguishability.
   Each remains in the Phase 4av friction section as an
   open low-reasoning follow-up. Candidates for a separate
   "Phase 4av polish follow-through" mini-phase OR fold-in
   to a later sub-batch. Reasoning level: **low** if
   standalone.
...
11. 🟡 **USER-FLAGGED External-output plot style presets**
    continues. Cross-ref Phase 4ao friction #6. User has
    reference Jupyter notebook code (paths TBD at session
    start). Reasoning level: **high**. Cross-refs item #3
    above — external presets are the first credible
    non-`"internal"` `engine` candidate in the wild.

12. 🟢 **CS-21 D3 lock relaxation precedent reinforced
    (Claude-surfaced, Phase 4ay artifact).** Continuation
    of Phase 4ax friction #11 (CS-75 D3 relaxed via step-5
    elicitation). Phase 4ay added a second precedent: CS-21
    D3's `pick_default_color` three-caller list was relaxed
    additively (the function's external contract preserved;
    internals rewritten to consult `active_palette()`).
    Two consecutive sub-batches now demonstrate that
    Phase-4aw locks enumerate the known-as-of-4aw surface
    area and step-5 elicitation is the canonical relaxation
    gate. No further bookkeeping needed.
    Reinforced again in Phase 4az: CS-75 D6 was held intact
    with an additive sub-clause for hover-less container
    widgets (see Phase 4az friction #4 below). Three
    consecutive sub-batches now follow the same shape.

### Friction points carried forward from Phase 4az

These are concrete obstacles the next Phase 4 session will hit.
Phase 4az landed the third sub-batch of the CS-75 Accessibility
umbrella — sub-axis C (Keyboard shortcuts first batch + new
`KEYBINDINGS.md` sister doc). Three phase commits + this
bookkeeping commit. 1662 tests, all green (1620 baseline + 42
net new). PTMG_FORMAT_VERSION unchanged — Phase 4az first batch
...
2. 🟢 **`_toggle_row_selection` focus_set retrofit is a
   ...
   4az added `self.focus_set()` at the end of
   `ScanTreeWidget._toggle_row_selection` so the four
   keyboard shortcuts (F2 / Delete / Ctrl+G / Ctrl+Shift+G)
   bound on the Frame are reachable from the keyboard after
   a row click. This couples selection-toggling to focus
   management. A future Tab-cycling story (CS-75 D3
   extension — keyboard navigation between left-pane
   surfaces) would benefit from a more disciplined focus
   model (probably a per-tab focus manager that knows the
   intended traversal order). Low priority — the retrofit
   works; flag for the next sub-batch that grows the
   keyboard-nav scope. No register entry yet — surface as a
   candidate when sub-axis C extends to a second batch.

3. 🟢 **`event_generate` continues to be unreliable under
   the full suite (Phase 4az reinforcement of Phase 4ax
   caveat).** All Phase 4az shortcut tests assert behaviour
   via direct handler invocation + binding-script presence
   rather than synthetic key events. This matches the
   existing CS-76 caveat but the test pattern is now
   load-bearing on two recipes (Escape-dismiss + keyboard
   shortcuts). If a future Tk update fixes the underlying
   flake, the tests could be tightened to drive the bindings
   end-to-end via `event_generate` — but as of Phase 4az the
   indirect assertion is the only reliable path. No
   bookkeeping action needed.

4. 🟢 **CS-75 D6 additive sub-clause for hover-less widgets
   (Claude-surfaced + lock-managed, Phase 4az artifact).**
   `bind_shortcut` does NOT call `attach_shortcut_tooltip`
   because the Phase 4az first batch binds on tree-level
   container widgets (`ScanTreeWidget` Frame) with no
   natural Tooltip hover surface. CS-75 D6 is preserved as
   "Tooltip-only discoverability for widgets that HAVE a
   hover surface; the Accessibility-tab table covers
   widgets that do not." Future shortcut-bearing widgets
   (buttons, comboboxes — the Phase 4ax `NodeStylesDialog`
   precedent) still wire both helpers side-by-side.
   Reinforces the Phase 4ax / 4ay step-5 relaxation pattern
   (locks enumerate the known-as-of-4aw surface area;
   step-5 elicitation is the relaxation gate — extended
   here to "Claude-surfaced architecture findings during
   implementation can additively extend a lock so long as
   the change is captured in the next bookkeeping commit").

5. 🟢 **`SHORTCUT_REGISTRY` is module-level mutable state
   (Claude-surfaced, Phase 4az artifact).** Idempotent on
   `(category, key)` so no real bloat risk, but every test
   that constructs a `ScanTreeWidget` (or otherwise calls
   `bind_shortcut`) populates the registry for the process
   lifetime. Tests that need a clean baseline must
   snapshot + restore (the pattern landed in
   `TestPlotConfigDialogShortcutsLabelFramePhase4az`).
   Same pattern as `node_styles._active_palette_name`
   (Phase 4ay). No bookkeeping action needed; flagged for
   awareness when Phase 4ba's font-scale work adds a
   second piece of module-level accessibility state.

6. 🟡 **USER-FLAGGED — `[engine engine_version]` provenance
   bracket display when uniform across rows (continued
   from Phase 4ay friction #3).** No work in Phase 4az;
   still open. Two improvements: (a) suppress the bracket
   when uniform across visible rows; (b) adopt a meaningful
   version-bump policy. Cross-refs CS-42 (Tooltip is the
   natural home if hover-only is chosen). Reasoning level:
   **low-medium** for display suppression; **medium** for
   the version-bump policy discussion. No register entry yet
...
8. 🟢 **Out-of-scope Toplevel inventory carry-forward
   ...
   friction #4).** Phase 4az added no new wirings (focused
   on keyboard shortcuts). Remaining sites for future
   Escape-recipe expansion: `plot_widget.py` 7 sites,
   `xas_analysis_tab.py` 2 sites, `ledge_normalizer.py` 1
   site, `nbo_viewer_app.py` 13 sites (sibling app; out of
   scope). Reasoning level: **low** if pursued standalone,
   **medium** if rolled into Phase 4ba as opportunistic
   cross-cut wiring.

9. 🟢 **Hard-coded Tooltip text on the NodeStylesDialog
   Combobox (continued from Phase 4ay friction #7 / Phase
   4ax friction #6).** Phase 4az added no new gestures on
   that Combobox, so the Tooltip text rotation precedent is
   not exercised. Still open: if a later sub-batch adds
   Ctrl+Home / Ctrl+End on the Combobox, rotate via
   `self._combobox_tooltip.update_text(...)`. No register
   entry.

10. 🟢 **Phase 4av carry-forward items #2 / #3 / #4 still
    open (continued from Phase 4ay friction #8 / Phase 4ax
    friction #7 / Phase 4aw friction #5).** (#2) "Reset"
    button label semantics shift; (#3)
    `_set_universal_disabled` root readonly-Combobox latent
    bug; (#4) DISCARDED glyph distinguishability. Each
    remains open as a low-reasoning follow-up. Candidates
    for a separate "Phase 4av polish follow-through"
    mini-phase OR fold-in to a later sub-batch.

11. 🟡 **USER-FLAGGED Axis nomenclature rename** continues.
    Cross-ref Phase 4ao friction #4. Massive cross-codebase
    rename. Reasoning level: **extra-high**.

12. 🟡 **USER-FLAGGED Rich-text axis labels (mathtext)**
    continues. Cross-ref Phase 4ao friction #5. Reasoning
    level: **medium**.

13. 🟡 **USER-FLAGGED External-output plot style presets**
    continues. Cross-ref Phase 4ao friction #6. User has
    reference Jupyter notebook code (paths TBD at session
    start). Reasoning level: **high**. Cross-refs Phase 4az
    friction #6 above — external presets are the first
    credible non-`"internal"` `engine` candidate in the
    wild.

14. 🟢 **BACKLOG.md doc-version footer skipped in Phase
    4ay bookkeeping (Claude-surfaced, Phase 4az artifact).**
    Phase 4ay's hand-off brief claimed the BACKLOG footer
    was bumped 1.49 → 1.50 in commit `566a2dd`, but the
    actual commit left it at 1.49 (Phase 4az step-1
    verification caught the drift). User direction at
    Phase 4az step 1: treat 1.49 as the real baseline;
    Phase 4az bookkeeping takes the bump 1.49 → 1.50
    (alongside COMPONENTS 1.51 → 1.52) so version history
    stays monotonic without an out-of-phase micro-commit on
...
1. ~~🟡 **USER-FLAGGED — extend per-axis font control to the primary
   ...
   Resolved in Phase 4bc (CS-81). Both primary tabs got their own
   Font LabelFrame (Size/Bold lockstep-shared with the Global control
   via idempotent `_ensure_flat_*_var`; new Italic + Family); a font
   FAMILY picker (`available_font_families()` + sentinel + graceful
   fallback for uninstalled families) landed on every tab. `font_weight`
   was already covered by Bold; remaining `font_style` beyond italic
   (oblique) was not requested.
...
3. 🟢 **Primary roles carry inert per-axis font keys (Claude-surfaced,
   ...
   or edited (no widget builds them).~~ **Partially resolved in Phase
   4bc (CS-81):** the primary `axis_label_font_italic` slot is now
   read+edited (the new primary Italic checkbox) and the new
   `axis_label_font_family` slot too. The primary `axis_label_font_size`
   / `_bold` slots remain DELIBERATELY inert — Size/Bold stay flat-key
   authoritative (the CS-80 lock the resolver still honours), and the
   primary Font frame's Size/Bold share the flat Global Tk var rather
   than the per-axis slot. A future "make per-axis primary size/bold
   authoritative" would relax that lock + need a flat→per-axis
...
### Friction points carried forward from Phase 4bc

These are concrete obstacles the next Phase 4 session will hit.
Phase 4bc landed CS-81 — per-axis font extended to the two PRIMARY
tabs (their own Font LabelFrame: Size/Bold lockstep-shared with the
Global control, new Italic + Family) + a font FAMILY picker on every
axis tab (`available_font_families()` cached lazy matplotlib scan,
`"(default)"` sentinel, graceful fallback for uninstalled families).
One additive per-axis key (`axis_label_font_family`, `_AXIS_KEYS`
15 → 16); PTMG_FORMAT_VERSION unchanged. Four code/test commits + this
bookkeeping. 1746 tests, all green (1716 baseline + 30 net new).
**Do not fix until the relevant subsequent Phase 4 session.**

1. 🟡 **USER-FLAGGED satisfied for labels; title/tick/legend font
   control still global (Claude-surfaced, CS-81 scope boundary).** The
   per-axis Font frame (size/bold/italic/family) now covers every axis
   LABEL on every tab. The plot TITLE, TICK labels and LEGEND remain
   global-only and family-less (the CS-80 tier-1 boundary, unchanged):
   they read flat `cfg.get(...)` sizes and never receive a `fontfamily`.
...
3. 🟢 **Family picker offers all installed families, unfiltered + no
   ...
   lists every `available_font_families()` entry (235 on the dev host)
   with no curated common-fonts shortlist, no per-entry preview, and no
   live in-dialog sample. Functional but long. A future polish could add
   a curated head + a preview Label. No register entry.

4. 🟢 **Primary size/bold stay flat-key authoritative (CS-81 lock,
   residual of 4bb friction #3).** The primary Font frame's Size/Bold
   share the flat Global `xlabel/ylabel_font_size`/`_bold` Tk vars; the
   per-axis primary `axis_label_font_size`/`_bold` slots stay inert by
   design. Making per-axis primary size/bold authoritative would relax
   the locked flat-bridge + need a flat→per-axis migration; not
   requested. Flagged so the next implementer knows the split.

5. 🟢 **Secondary-X tick label still hardcoded 8pt (continued from
   Phase 4bb friction #4).** Unchanged — ticks remain out of the
   label-only scope. Absorbed by a future per-axis tick-size sub-batch.
   No register entry.

6. 🟡 **USER-FLAGGED — `[engine engine_version]` provenance bracket
   display when uniform across rows (continued from Phase 4bb friction
   #5 / 4ba #5 / 4az #6 / 4ay #3).** No work in Phase 4bc; still open.
...
7. 🟡 **USER-FLAGGED — Escape-dismiss expansion follow-on (continued
   from Phase 4bb friction #6 / 4ba #7 / 4az #8 / 4ay #6).** No work in
   Phase 4bc. Remaining CS-76 Escape-recipe sites: `plot_widget.py` 7,
   `xas_analysis_tab.py` 2, `ledge_normalizer.py` 1. ~10 wirings + 1
   source-inventory sentinel. Reasoning level: **low-medium**.

8. 🟢 **Phase 4av carry-forward items #2 / #3 / #4 + `_toggle_row_
   selection` focus_set retrofit still open (continued from Phase 4bb
   friction #7 + #8).** (#2) "Reset" button label semantics; (#3)
   `_set_universal_disabled` root readonly-Combobox latent bug; (#4)
   DISCARDED glyph distinguishability; plus the focus_set / `event_
   generate` test caveat. The CS-81 Tk tests likewise assert via direct
   `var.set` + state inspection (no synthetic key events). A
   low-reasoning polish mini-phase closes the queue.

9. 🟡 **USER-FLAGGED long-running items continue.** Axis nomenclature
   rename (**extra-high**); Rich-text / mathtext axis labels
   (**medium** — interacts with CS-80/CS-81, since a rich-text label
