...
| ⏳ | 🔴 | **Right-sidebar canvas-width binding + responsive helper canvas-Configure rerun (USER-FLAGGED)** | USER-FLAGGED at start of Phase 4p. The Phase 4d / 4n responsive layout (CS-26) was correct in *threshold logic* but wired to the wrong width signal: the row's natural width, which is content-driven (longest visible label sets the inner-frame width via Tk's auto-sizing of `_rows_frame` inside `_scroll_canvas`). Two confirmed symptoms: (a) with one node, the row stays collapsed even at 600+ px sidebar width because the inner frame never grows past the single label; (b) with two nodes, dragging the sidebar narrower does NOT recollapse the row because `<Configure>` only fires per row when the row itself resizes, and the row only resizes when content changes. Likely shape (Phase 4p, CS-30): bind `_scroll_canvas` `<Configure>` and call `self._scroll_canvas.itemconfig(self._rows_window, width=event.width)` so the inner frame tracks canvas width regardless of content; in the same handler, walk every row and call `_apply_responsive_layout(nid, frm)` so a width change triggers reflow in both directions. Touches `scan_tree_widget._build_chrome` plus regression tests (canvas-resize widens then narrows; threshold crossings observed in both directions; single-node sidebar uses the full canvas width). Pairs with the earlier Phase 4n carry-forward "Threshold-band caching" (above) — once CS-30 lands, the helper's call frequency grows so the caching pass becomes more attractive |
| ⏳ | 🔴 | **Suppress identical re-applies (param-equality gate on Apply) (USER-FLAGGED)** | USER-FLAGGED at start of Phase 4p. Today an Apply gesture creates a new PROVISIONAL OperationNode + DataNode pair regardless of whether an existing PROVISIONAL sibling has the same params. Clicking Apply twice with identical params produces two PROVISIONAL siblings sharing one parent → the right-side ScanTreeWidget collapses them into a sweep group (per the Phase 1 rule) even though the second click was a duplicate, not a parameter sweep. Architecturally a "sweep" is "an operation across a parameter range" (see ARCHITECTURE.md §5.4), so identical params should not qualify. Likely shape (Phase 4p, CS-31): a new helper `_find_provisional_sibling(parent_id, op_type, params) -> Optional[str]` (or a graph-level method, depending on where param equality is most natural to express) that walks PROVISIONAL OperationNodes consuming `parent_id` and returns the first whose `params` dict equals the proposed `op_params`. Each apply site (`uvvis_tab._apply_baseline` plus the apply methods inside `uvvis_normalise.py` / `uvvis_smoothing.py` / `uvvis_peak_picking.py` / `uvvis_second_derivative.py`) calls the helper after params are validated and before `compute()` runs. On hit: status message "*<op> with these parameters already exists for <parent label> — no new node created.*", no graph mutation, return None. Tests: identical re-apply produces no new node; param-tweaked re-apply DOES create a new node and IS grouped as a sweep (because now the siblings differ); status message shape is asserted. Pairs with CS-32 (per-variant gestures) — both are needed for the sweep workflow to feel coherent |
| ⏳ | 🔴 | **Inline expansion + per-variant gestures on sweep-group rows (CS-04 §6.3 follow-through) (USER-FLAGGED)** | USER-FLAGGED at start of Phase 4p; promotes the existing Phase 2 carry-forward "Sweep group inline expansion" + the existing Phase 4k entry "Per-variant gestures on sweep-group rows" (both above). The architecture sketch (ARCHITECTURE.md §6.3) describes "Expanding shows all variants ranked by fit metric", but the Phase 2 sweep-group leader row currently exposes only `✕all` and the user cannot commit / discard / restyle individual variants. Likely shape (Phase 4p, CS-32): the leader row gets a chevron `▸ <parent label> · sweep (N variants) [✕all]`. Click → `▾` and the variants render inline beneath as full-chrome rows (state · swatch · ☑ · label · ⌥n · ⚙ · → · ✕) by reusing `_populate_node_row`. Expansion state persists across rebuilds via `self._expanded_sweep_groups: set[str]` keyed by parent_id, mirroring the existing `_expanded_history` pattern. A `🔒` (commit) button is added next to `✕` on member rows so committing one variant doesn't require dropping into the right-click menu. Tests: chevron toggles `▸/▾`; inline members render with full controls; committing one member promotes it out of the group (drops below the 2-member threshold → group dissolves); styling one member doesn't bleed onto siblings. Promotes BACKLOG row 187 from ⏳ to ✅ at landing |
